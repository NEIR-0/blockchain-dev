...
import abi from "../../../../ABI/public_wallet.js";
...
function PublicWallet() {
    ...
    try {
      if (typeof window.ethereum !== "undefined") {
        ...
        const contractAddress = import.meta.env
          .VITE_ADDRESS_CONTRACT_PUBLIC_WALLET;
        // console.log(abi, "????");
  ...
  const withdraw = async () => {
    ...
    const tx = await contract.connect(provide);
    const response = await tx.withdrawAmount(
  ...
  return (
    <>
      <section className="w-full h-screen bg-yellow-300 pt-14 flex items-center justify-center flex-col">
        <h1 className="text-[30px] font-bold mb-10">public wallet </h1>
        ...
        <div className="w-1/2 h-fit flex items-center justify-center mt-10 gap-2">
          ...
          <h1>
            Our balace: <span>{balance ? balance : "loading..."}</span>
          </h1>
        </div>
      </section>
    </>
